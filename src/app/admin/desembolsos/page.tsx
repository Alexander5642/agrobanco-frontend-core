...
import { desembolsarCredito, rechazarDesembolso } from './actions'

export const dynamic = 'force-dynamic';

export default async function AdminDesembolsosPage({ searchParams }: { searchParams?: { q?: string } }) {
  const db = readDB()

  const q = searchParams?.q?.toLowerCase() || ''

  // Jalar todos los créditos aprobados listos para desembolso
  ...
  return (
    <div className="max-w-6xl mx-auto flex flex-col gap-6 p-8">
      <div className="mb-4">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Aprobación y Desembolso</h1>
        <p className="text-gray-500">Créditos aprobados pendientes de transferencia de fondos a la cuenta del cliente</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-medium text-gray-500">Pendientes de Desembolso</p>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-4 border-b border-gray-50 flex justify-between items-center bg-gray-50/50">
          <h3 className="font-bold text-gray-900">Bandeja de Desembolsos</h3>
        ...
        <div className="flex flex-col overflow-y-auto max-h-[600px]">
          {creditos.length === 0 ? (
            <div className="p-12 text-center text-gray-500">No hay créditos pendientes de desembolso.</div>
          ) : (
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-700">
                      Monto a transferir: <span className="font-bold text-brand">{formatMoney(credito.monto)}</span>
                    </p>
                  </div>
                </div>

                <div className="flex flex-col items-end gap-2">
                  <span className="px-4 py-1.5 font-bold text-xs rounded-lg uppercase tracking-wider bg-green-100 text-green-800 border border-green-200">
                    {credito.estado}
                  </span>
                  
                  <div className="flex gap-2 mt-2">
                    <form action={rechazarDesembolso}>
                    ...
                    <form action={desembolsarCredito}>
                      <input type="hidden" name="credito_id" value={credito.id} />
                      <button className="px-4 py-2 bg-brand text-white hover:bg-brand-dark font-bold rounded-xl text-sm shadow-sm transition-colors flex items-center gap-2">
                        Desembolsar <Activity className="w-4 h-4" />
                      </button>
                    </form>
                  </div>
                </div>
